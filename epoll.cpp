...
std::unordered_map<int, std::shared_ptr<HttpRequest>> Epoll::fd2req_;

int Epoll::epollInit(int flags)
{
...
int Epoll::epollAdd(int fd, std::shared_ptr<HttpRequest> request, int events)
{
...
int Epoll::epollMod(int fd, std::shared_ptr<HttpRequest> request, int events)
{
    struct epoll_event event;
    event.data.fd = fd;
    event.events = events;
...
int Epoll::epollDel(int fd, int events)
{
    struct epoll_event event;
    event.data.fd = fd;
    event.events = events;
    ...
    // 处理完请求删除节点
    auto fd_iter = fd2req_.find(fd);
    if (fd_iter != fd2req_.end())
        fd2req_.erase(fd_iter);
    return 0;
}

int Epoll::eopllWait(int listen_fd, int max_events, int timeout)
{
...
std::vector<std::shared_ptr<HttpRequest>> Epoll::getEventsRequest(int listen_fd, int events_count)
{
    ...
    for (int i = 0; i < events_count; ++i)
    {
        ...
        else
        {
            ...
            std::shared_ptr<HttpRequest> cur_req(fd2req_[fd]);
            // 处理该请求时先将计时器分离
            cur_req->sepereteTimer();
            request.push_back(cur_req);
            auto fd_ite = fd2req_.find(fd);
            if (fd_ite != fd2req_.end())
                fd2req_.erase(fd_ite);
        }
    }
    return request;
}
...
{
    std::shared_ptr<HttpRequest> req = std::static_pointer_cast<HttpRequest>(request);
    req->handleRequest();
}

void Epoll::handleEvents(int listen_fd, int events_count)
{
