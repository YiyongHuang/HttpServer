...
std::vector<std::shared_ptr<HttpRequest>> Epoll::getEventsRequest(int listen_fd, int events_count, const std::string path)
{
    ...
    for (int i = 0; i < events_count; ++i)
    {
        ...
        {
            acceptConnection(listen_fd, path);
        }
        else
        {
            // 排除错误事件
...
void Epoll::acceptConnection(int listen_fd, const std::string path)
{
...
{
    std::vector<std::shared_ptr<HttpRequest>> request = getEventsRequest(listen_fd, events_count, path_);
    if (request.size() > 0)
    {
